...
# Classe criada para definir os metodos comuns a recepção e o dentista
class Clinica:
    ...
    # Método que encerra a sessão iniciada
    def encerrarSessao(self, sessaoAtiva):
        if len(self.fila) == 0:
            ...
            print('Ainda há pacientes na fila de atendimento')

# Classe criada para definar os metodos da Recepção
class Recepção(Clinica):
    ...
    def listarSessoes(self):
        for s in self.sessoes:
            print(f'Data: {s['Data']} | Horario: {s['Horario']} | Tempo da sessao: {s['Tempo da Sessao']}', end= '')
            print()

    # Método que lista os pacientes cadatrados no sistema
    def listarPacientes(self):
        for p in self.pacientes:
            print(f'Nome: {p['nome']} | CPF: {p['CPF']} | ID: {p['id']}', end= '')
            print()  
    ...
    def marcarHorario(self, CPF):
        ...
        if sessao != None:
            '''Loop percorre lista de pacientes e adiciona a variavel "paciente" na lista "confirmaçãoPaciente" caso o paciente seja 
            encontrado'''
            confirmaçãoPaciente = []
            for p in self.pacientes:
                if p['CPF'] == CPF:
    ...
    def horarioPacientes(self, CPF):
        ...
        if sessoesDoPaciente:
            ...
            for i in sessoesDoPaciente:
                print(f'Data: {i['Data']} | Horario: {i['Horario']} | Tempo da sessao: {i['Tempo da Sessao']}\n')
                criarlinha()
...
# Classe criada para definar os metodos do Dentista 
        ...
        if nome != None:
            ...
            for p in self.pacientes:
                if p['nome'] == nome:
                    ...
                    anotação = (f'{prontuario} | Data: {data}, | Nome: {nome}')
